...
function Navbar() {
  ...
  const navLinks = [
    { title: "Services", target: "about" },
    { title: "Our Work", target: "our-work" },
    { title: "About us", target: "about" },
    { title: "Insights", target: "insights" },
    { title: "Contacts", target: "contacts", isContact: true },
  ];
  ...
  return (
    <motion.nav 
      ...
      className={`fixed z-[999] w-full px-12 lg:px-20 font-neue flex justify-between items-center transition-all duration-300 ${
        scrolled 
          ? 'py-4 bg-zinc-900/80 backdrop-blur-lg border-b border-zinc-800/80 shadow-2xl' 
          : 'py-8 bg-transparent'
      }`}
    >
      ...
      <div 
        ...
        className='logo cursor-pointer group transition-transform duration-300 hover:scale-105'
      >
        <svg width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg" className='text-white fill-current transition-colors duration-300 group-hover:text-[#CDEA68]'>
      ...
      <div className='links flex items-center gap-10'>
        {navLinks.map((item, index) => (
          <button
            ...
            className={`relative font-light text-lg capitalize transition-colors duration-300 py-1 ${
              item.isContact ? "ml-16 md:ml-32" : ""
            } group cursor-pointer text-zinc-100 hover:text-white`}
          >
            {item.title}
            {/* Animated Underline */}
            <span className="absolute bottom-0 left-0 w-full h-[2px] bg-[#CDEA68] scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300 ease-out rounded-full" />
          </button>
        ))}
      </div>
    </motion.nav>
